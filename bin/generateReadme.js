...
const { isNotNilOrEmpty } = require('@paravano/utils');
...
const index = async () => {
  ...
  const functions = Array.from(readdirSync(join(src))).filter(x => !x.startsWith('_') && exclude.indexOf(x) === -1);
  
  for (const func of functions) {
    const file = readFileSync(join(src, func, 'index.ts'), 'utf8');
  ...
  const templateData = {
    categories,
    utils: generateIndividualReadMes(utils, packageData.name),
    package: packageData,
  };
...
const generateIndividualReadMes = (utils, packageName) => utils.map(util => generateTable(util, packageName));
